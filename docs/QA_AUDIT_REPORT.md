...
Changed: backend ETA/speed calculations and regression tests; shared stop-time and driver GPS helpers and tests; Driver home, Student tracking and Admin live arrival summaries; scoped Driver CSS; browser regression expectations; this report. No new secrets, configuration variables, packages or database migration are required.

Published release `1bdd371ca4db74d4de2ca51eca68ef3ac59586bd` to the existing GitHub `main` and audit branches. Vercel production `dpl_GZmNNviartjwh6KsMHXVrr6rspcJ` is READY with the live alias assigned; Render `dep-danp6fcs728c73b1ill0` is live, completed at 13:33:50 IST. Post-release read-only checks: API health 200/ok, unsigned student transit 401, correct production CORS origin. No production trip, account or passenger record was deliberately changed and no provider received test coordinates. Existing active trips can use the correction after a fresh GPS fix; restarting them is unnecessary. Prior release `cc7032e` remains the rollback reference. Hosted visual/device testing is still unverified for the reasons above. The local full application also started successfully at `http://127.0.0.1:5177` with isolated API health 200; it does not contain production accounts.
