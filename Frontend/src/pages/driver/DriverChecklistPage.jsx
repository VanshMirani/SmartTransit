import { AlertTriangle, ArrowLeft, Check, CheckCircle2, ClipboardCheck, LoaderCircle, Play, ShieldCheck } from 'lucide-react';
...
export function DriverChecklistPage() {
    const { activeTrip, checklist, tripLoadError, toggleCheck, startTrip, tripStatus } = useDriverOperations();
    const [confirming, setConfirming] = useState(false);
    const [starting, setStarting] = useState(false);
    const [startError, setStartError] = useState("");
    const dialogRef = useDialogFocus(confirming, () => { if (!starting) setConfirming(false); });
    ...
    if (tripLoadError)
        return <section className="staff-state-card"><span><AlertTriangle /></span><h1>Assigned trip could not load</h1><p>{tripLoadError}</p><button className="button staff-primary-button" type="button" onClick={() => window.location.reload()}>Refresh driver dashboard</button><Link className="staff-back-link" to="/driver"><ArrowLeft /> Back to home</Link></section>;
    if (tripStatus === 'completed')
        return <section className="staff-state-card"><span><ClipboardCheck /></span><h1>Choose the next trip direction</h1><p>Select Morning pickup or Return trip from the driver home screen before starting another checklist.</p><Link className="button staff-primary-button" to="/driver">Back to driver home</Link></section>;
    ...
    <section className="checklist-trip-summary"><div><span className="staff-square-icon"><ClipboardCheck /></span><span><small>Today’s assignment</small><strong>{activeTrip.routeCode} · {activeTrip.routeName}</strong><em>{tripDirectionLabel(activeTrip.direction)} trip · {activeTrip.registration} · Departure {activeTrip.scheduledStart}</em></span></div></section>
    ...
    {confirming && <div className="staff-modal-backdrop" role="presentation"><section ref={dialogRef} tabIndex={-1} className="staff-modal" role="dialog" aria-modal="true" aria-labelledby="start-title"><span className="staff-modal__icon"><Play /></span><h2 id="start-title">Start this trip?</h2><p>GPS sharing will begin automatically and your live location will become visible to authorized students and transport operators.</p><div className="modal-trip-row"><span><strong>{activeTrip.routeCode}</strong>{tripDirectionLabel(activeTrip.direction)} · {activeTrip.routeName}</span><span><strong>{activeTrip.scheduledStart}</strong>Scheduled start</span></div>{startError && <p className="field-error" role="alert">{startError}</p>}<div className="staff-modal__actions"><button className="button button--secondary" onClick={() => setConfirming(false)} disabled={starting}>Cancel</button><button className="button staff-primary-button" onClick={start} disabled={starting}>{starting ? <><LoaderCircle className="spin"/> Starting…</> : <><Play /> Confirm & start</>}</button></div></section></div>}
  </div>;
}
