...
export function DriverTripPage() {
    const { tripStatus, activeTrip, stops, gpsUpdatedAt, endTrip } = useDriverOperations();
    ...
    const nextStop = stops.find((stop) => stop.id === activeTrip.nextStopId) ??
        ...
        stops[0];
    const busPosition = nextStop.coordinates;
    return <div className="driver-active-page"><StaffPageHeading eyebrow={`${activeTrip.routeCode} · Active trip`} title="Stay focused. Drive safely." description="Only essential trip information is shown while you’re moving." status={<span className="staff-status staff-status--active"><i /> GPS active</span>}/>
