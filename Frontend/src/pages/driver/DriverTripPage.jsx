...
export function DriverTripPage() {
    ...
    <section className="active-trip-hero"><div><span><Navigation /></span><div><small>Next stop</small><h2>{nextStop.name}</h2><p>{activeTrip.remainingDistance} remaining · ETA {activeTrip.nextStopEta}</p></div></div><div className="active-trip-countdown"><small>Live ETA</small><strong>{activeTrip.nextStopEta}</strong><span>{gpsUpdatedAt === 'Waiting for driver phone' ? 'waiting for GPS' : `GPS updated ${gpsUpdatedAt}`}</span></div></section>
