...
export function StaffLayout({ role }) {
    ...
    <header className="staff-topbar"><button ref={triggerRef} className="staff-menu" onClick={() => setOpen(true)} aria-label="Open navigation" aria-expanded={open} aria-controls="staff-navigation"><Menu /></button><NavLink className="staff-brand" to={`/${role}`}><BusFront /><span>Smart<strong>Transit</strong></span></NavLink><div className="staff-topbar__role"><span>{user?.initials}</span><div><strong>{user?.name}</strong><small>{config.label}</small></div></div></header>
