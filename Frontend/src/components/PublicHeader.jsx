...
export function PublicHeader() {
    ...
    return (<header className="site-header">
      <div className="container header-inner">
        <Brand light/>
        <button ref={triggerRef} className="menu-button" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="public-nav" aria-label={open ? 'Close navigation' : 'Open navigation'}>
          {open ? <X /> : <Menu />}
        </button>
        <nav ref={navigationRef} id="public-nav" className={`public-nav ${open ? 'public-nav--open' : ''}`} aria-label="Primary navigation">
